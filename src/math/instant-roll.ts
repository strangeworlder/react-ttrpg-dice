...
/**
 * Core die-rolling logic shared by both instant roll paths.
 * Appends results for one parsed notation group to `rolls`.
 */
function rollParsedGroup(
  ...
  for (const group of parsed.groups) {
    for (let i = 0; i < group.count; i++) {
      if (group.type === 'd100') {
        ...
        rolls.push({ type: 'd100', value, isMax: value === 100, isMin: value === 1, group: groupLabel });
      } else {
...
export function instantRoll(parsed: ParsedNotation, registry: DieRegistry): RollResult {
  ...
  rollParsedGroup(parsed, registry, rolls);
...
export function instantGroupedRoll(groups: DiceGroup[], registry: DieRegistry): RollResult {
  ...
  for (const dg of groups) {
    ...
    rollParsedGroup(parsed, registry, rolls, dg.label);
  }
